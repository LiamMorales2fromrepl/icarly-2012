...
// Added change_header_photo event 2/10/11 -GV
...
function bumperize(url) {
  return "/bumper.html?url=" + escape(url);  
}
...
function bumper_if_no_ad_cookie() {
  ...
  if (document.referrer.indexOf('.icarly.com/') == -1) { // no bumper check if coming from outside
    ...
    window.location = bumperize(document.location);
}
...
// jquery facade for cookie functions
$.cookie = function(name, value, num_days) {
  if (typeof value == 'undefined') {
    return get_cookie(name); 
  } else {
    var num_days = typeof num_days != 'undefined'? num_days: 0;
    return set_cookie(name, value, num_days, '/'); 
  }
}
...
$(document).bind('change_header_photo', function(){
    ...
    // add a class with the random index to the header, or cast for homepage 
    var photo_class = 'random' + random_index;
    (window.location && location.pathname) && (location.pathname.match(/\//g).length < 2) && (photo_class = 'all');   
    $('#header_photo').attr('class',photo_class);
});
